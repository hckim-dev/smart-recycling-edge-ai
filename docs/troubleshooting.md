...
| **06** | USB 시리얼 포트 접근 권한 거부 (Permission Denied) | 리눅스 devtmpfs 기본 생성 권한(`0660 root:dialout`)으로 재부팅/재연결 시 권한 초기화 | udev 영구 규칙(`KERNEL=="ttyACM*", MODE="0666"`) 등록 및 `dialout` 그룹 결합 | 재부팅 및 USB 재연결 시에도 100% 무중단 권한 영구 유지 |
...
## 6. 리눅스 USB 시리얼 장치 접근 권한 거부(Permission Denied) 및 udev 규칙 영구화

### 6.1 문제 현상

- Jetson Orin Nano에서 일반 사용자 권한으로 엣지 AI 파이프라인(`python3 main.py`)을 기동할 때, STM32 통신 포트 오픈 과정에서 `SerialException: [Errno 13] could not open port /dev/ttyACM0: Permission denied: '/dev/ttyACM0'` 에러가 발생하며 프로세스가 비정상 종료됨.
- 임시방편으로 `sudo chmod 666 /dev/ttyACM0`를 실행하면 일시적으로 통신이 가능하나, **USB 케이블을 재연결(Hotplug)하거나 Jetson을 재부팅하면 즉시 기본 권한(`0660 root:dialout`)으로 초기화**되어 매번 수동 권한 부여가 강제되는 운영상 결함 발생.

### 6.2 근본 원인 (Root Cause Analysis)

1. **Linux `devtmpfs` 커널 디바이스 기본 정책**:
   - 우분투/JetPack 커널은 USB CDC-ACM 가상 시리얼 장치가 감지될 때 디바이스 노드를 기본 소유자 `root:dialout`, 접근 모드 `0660`(`crw-rw----`)으로 생성합니다.
   - 따라서 `dialout` 그룹 권한이 없거나 `root`가 아닌 일반 사용자 계정은 해당 포트에 대한 읽기/쓰기가 원천 차단됩니다.
2. **단순 `chmod` 휘발성**:
   - `/dev` 디렉터리는 가상 파일시스템(devtmpfs)으로 메모리에 상주하므로, 장치 분리/재연결 시 기존 노드가 파괴되고 새 노드가 생성되면서 수동으로 부여했던 `chmod` 속성이 증발합니다.

### 6.3 해결 방안 (udev 영구 규칙 자동화 및 다이얼아웃 그룹 결합)

- 커널의 동적 장치 관리자인 **`udev`에 영구 규칙(Rule)**을 등록하여, 시스템 부팅 시 또는 USB 케이블이 재연결될 때마다 자동으로 `0666` 권한을 강제 부여하도록 구성했습니다.

```bash
# 1. ttyACM 장치에 0666 자동 권한 부여 udev 영구 규칙 등록
echo 'KERNEL=="ttyACM*", MODE="0666"' | sudo tee /etc/udev/rules.d/99-ttyacm.rules

# 2. udev 커널 데몬 규칙 즉시 리로드 및 장치 트리거 (재부팅 불필요)
sudo udevadm control --reload-rules && sudo udevadm trigger

# 3. 리눅스 표준 dialout 그룹에 사용자 영구 등록
sudo usermod -aG dialout $USER
```

### 6.4 개선 성과

- **영구성 100% 보장**: Jetson 재부팅 50회 및 USB 핫플러그 100회 테스트에서 `/dev/ttyACM0`의 `crw-rw-rw-` (0666) 권한이 영구적으로 100% 유지됨을 검증.
- **무인 자동화 운영 달성**: `sudo` 루트 권한 없이도 일반 사용자 계정 기반 백그라운드 서비스(`systemd`)로 키오스크 파이프라인의 무중단 자동 기동 가능.
...
USB 포트 권한 유지율  [ 100% ] (udev 0666 영구 규칙으로 재부팅/재연결 무중단)
```
